from application_client.client import (
    AsyncAPDU,
    SW_OK,
    NavigableConditions,
    Nearbackend,
    generic_test_sign,
)
from ragger.backend.interface import RAPDU
from ragger.navigator import Navigator


def test_sign_transfer_to_gas_key(
    firmware, backend, navigator: Navigator, test_name
):
    """
    transaction length: 
    Transaction {
        signer_id: AccountId(
            "c4f5941e81e071c2fd1dae2e71fd3d859d462484391d9a90bf219211dcbb320f"
        ),
        public_key: ed25519:EFr6nRvgKKeteKoEH7hudt8UHYiu94Liq2yMM7x2AU9U,
        nonce: 103595482000005,
        receiver_id: AccountId(
            "dc7e34eecec3096a4a661e10932834f801149c49dba9b93322f6d9de18047f9c"
        ),
        block_hash: Cb3vKNiF3MUuVoqfjuEFCgSNPT79pbuVfXXd2RxDXc5E
        actions: [
            TransferToGasKey(
                TransferToGasKeyAction(
                    public_key: ed25519:EFr6nRvgKKeteKoEH7hudt8UHYiu94Liq2yMM7x2AU9U,
                    deposit: NearToken::from_yoctonear(150000000000000000000),
                ),
            ),
        ],
    """
    client = Nearbackend(backend)
    chunks = [
        AsyncAPDU(
            data=bytes.fromhex(
                ""
            ),
            navigable_conditions=NavigableConditions(
                value=["Continue to actions"],
            ),
            expected_response=RAPDU(
                SW_OK,
                bytes()
            ),
        ),
        AsyncAPDU(
            data=bytes.fromhex(
                "8002805721859d462484391d9a90bf219211dcbb320f00009814440dab210800000000000000"
            ),
            navigable_conditions=NavigableConditions(
                value=["Sign"],
            ),
            expected_response=RAPDU(
                SW_OK,
                # signature
                bytes.fromhex(
                    "bfab71dfcaf8aff894ecfd2f33a76991570d14c68544cd8539578cf4b90bffd4e8468e8e42962b0630b1fe5573da85b71e9818aeffba4fc01dc877526030da00"
                ),
            ),
        ),
    ]

    generic_test_sign(client, chunks, navigator, test_name, firmware)


def test_sign_transfer_to_gas_key_secp(
    firmware, backend, navigator: Navigator, test_name
):
    """
    transaction length: 
    Transaction {
        signer_id: AccountId(
            "c4f5941e81e071c2fd1dae2e71fd3d859d462484391d9a90bf219211dcbb320f"
        ),
        public_key: ed25519:EFr6nRvgKKeteKoEH7hudt8UHYiu94Liq2yMM7x2AU9U,
        nonce: 103595482000005,
        receiver_id: AccountId(
            "dc7e34eecec3096a4a661e10932834f801149c49dba9b93322f6d9de18047f9c"
        ),
        block_hash: Cb3vKNiF3MUuVoqfjuEFCgSNPT79pbuVfXXd2RxDXc5E
        actions: [
            TransferToGasKey(
                TransferToGasKeyAction(
                    public_key: secp256k1:2xV3hzGShUE3X5jE9jmAyFC67GfgwAUo5FoBJ79Zh84Z5Ubdxy94Ka73EWwrFg5FbVYAvtdqJK77P6CAdyMkEnca,
                    deposit: NearToken::from_yoctonear(150000000000000000000),
                ),
            ),
        ],
    """
    client = Nearbackend(backend)
    chunks = [
        AsyncAPDU(
            data=bytes.fromhex(
                ""
            ),
            navigable_conditions=NavigableConditions(
                value=["Continue to actions"],
            ),
            expected_response=RAPDU(
                SW_OK,
                bytes()
            ),
        ),
        AsyncAPDU(
            data=bytes.fromhex(
                "800280574157c5008156da0909c4a281f5c8d9ee3de837534833badf7ad41a5e83071908af7d4f2ae835c9d9aceb48cfb47a4c96509b00009814440dab210800000000000000"
            ),
            navigable_conditions=NavigableConditions(
                value=["Sign"],
            ),
            expected_response=RAPDU(
                SW_OK,
                # signature
                bytes.fromhex(
                    "634187c00ca559a31fbe82b2f096c0261913aba3afcdbde67443dec9eab8dbe882423e0fb5ce4e25bc24f9cbe4c37eaf04df94cae409240247417b601cb12d08"
                ),
            ),
        ),
    ]

    generic_test_sign(client, chunks, navigator, test_name, firmware)


def test_sign_transfer_to_gas_key_ml_dsa(
    firmware, backend, navigator: Navigator, test_name
):
    """
    transaction length: 
    Transaction {
        signer_id: AccountId(
            "c4f5941e81e071c2fd1dae2e71fd3d859d462484391d9a90bf219211dcbb320f"
        ),
        public_key: ed25519:EFr6nRvgKKeteKoEH7hudt8UHYiu94Liq2yMM7x2AU9U,
        nonce: 103595482000005,
        receiver_id: AccountId(
            "dc7e34eecec3096a4a661e10932834f801149c49dba9b93322f6d9de18047f9c"
        ),
        block_hash: Cb3vKNiF3MUuVoqfjuEFCgSNPT79pbuVfXXd2RxDXc5E
        actions: [
            TransferToGasKey(
                TransferToGasKeyAction(
                    public_key: ml-dsa-65:,
                    deposit: NearToken::from_yoctonear(150000000000000000000),
                ),
            ),
        ],
    """
    client = Nearbackend(backend)
    chunks = [
        AsyncAPDU(
            data=bytes.fromhex(
                ""
            ),
            navigable_conditions=NavigableConditions(
                value=["Continue to actions"],
            ),
            expected_response=RAPDU(
                SW_OK,
                bytes()
            ),
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        bytes.fromhex(
            ""
        ),
        AsyncAPDU(
            data=bytes.fromhex(
                ""
            ),
            navigable_conditions=NavigableConditions(
                value=["Sign"],
            ),
            expected_response=RAPDU(
                SW_OK,
                # signature
                bytes.fromhex(
                    "686f1242b1872a7fbcd13a4579c1114ac92623c15ae029756b491cd3d96a945ae13fc682066fa07055943dfef043c7f6034428c1d3df7fae0c3d5edf95c04504"
                ),
            ),
        ),
    ]

    generic_test_sign(client, chunks, navigator, test_name, firmware)
